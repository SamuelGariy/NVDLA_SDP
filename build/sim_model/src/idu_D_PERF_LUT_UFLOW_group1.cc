#include <sdp.h>
bool sdp::decode_sdp_D_PERF_LUT_UFLOW_group1() {
...
void sdp::update_sdp_D_PERF_LUT_UFLOW_group1() {
auto local_var_1 = sdp_csb_data.range(31, 0);
auto local_var_1_nxt_holder = local_var_1;
sdp_group1_d_lut_uflow = local_var_1_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "D_PERF_LUT_UFLOW_group1 state updates:" << std::endl;
instr_update_log << "    sdp_group1_d_lut_uflow => " << std::hex << "0x" << sdp_group1_d_lut_uflow << std::endl; 
